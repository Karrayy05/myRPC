...
int main() {
	...
	while (1) {
		...
		if (is_stream) {
			...
			char tmp;
			recvfrom(sock,&tmp,1,MSG_PEEK,(struct sockaddr*)&cli,&len);
			client = sock;
		}
		...
		ssize_t r = is_stream
			...
			: recvfrom(client,buf,sizeof(buf)-1,0,(struct sockaddr*)&cli,&len);
		if (r<1) { if (is_stream) close(client); continue; }
		buf[r]=0;
		json_log(buf, INFO, LOG_PATH);

		json_object *jroot = json_tokener_parse(buf), *jlogin, *jcmd;
		...
		int code=1;
		char *result =	user_allowed(login)
				? run_command(cmd,&code)
				: strdup("User not allowed");

		json_object *jresp = json_object_new_object();
		json_object_object_add(jresp,"code",json_object_new_int(code));
		json_object_object_add(jresp,"result",json_object_new_string(result));
